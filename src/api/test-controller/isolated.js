    ...
    _run$ (fn) {
        ...
        return this._enqueueTask('run', () => {
            return async () => {
                ...
                try {
                    // Redirect the global `t` and its commands at the isolated tab for the
                    // duration of the callback. Direct mode is required because we are already
                    // inside an executionChain task — re-appending would deadlock.
