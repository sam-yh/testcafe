...
test('cookies API set, get, delete', async t => {
    const t2 = await t.openIsolatedSession();

    await t2.navigateTo('http://localhost:3000/fixtures/isolated-sessions/pages/index.html');
    await t2.setCookies({ name: 'api-cookie', value: 'api-value' }, 'http://localhost:3000/');

    const documentCookie = await t2.eval(() => document.cookie);

    await t.expect(documentCookie).contains('api-cookie=api-value');

    const cookies = await t2.getCookies('api-cookie', 'http://localhost:3000/');

    await t
        .expect(cookies.length).eql(1)
        .expect(cookies[0].value).eql('api-value');

    await t2.deleteCookies('api-cookie', 'http://localhost:3000/');

    const afterDelete = await t2.eval(() => document.cookie);

    await t.expect(afterDelete).notContains('api-cookie');
});

test('cookie expires is respected', async t => {
    const t2 = await t.openIsolatedSession();

    await t2.navigateTo('http://localhost:3000/fixtures/isolated-sessions/pages/index.html');

    await t2.setCookies({ name: 'expired-cookie', value: 'x', expires: new Date(Date.now() - 86400000) }, 'http://localhost:3000/');
    await t2.setCookies({ name: 'future-cookie', value: 'y', expires: new Date(Date.now() + 86400000) }, 'http://localhost:3000/');

    const documentCookie = await t2.eval(() => document.cookie);

    await t
        .expect(documentCookie).notContains('expired-cookie')
        .expect(documentCookie).contains('future-cookie=y');
});
